...
impl TexasHoldemEngine {
    ...
    fn active_players_count(&self) -> usize {
        self.players.iter().filter(|p| !p.folded && !p.all_in).count()
    }
    ...
    fn all_bets_equal(&self) -> bool {
        let active_bets: Vec<u32> = self.players
            .iter()
    ...
    fn start_new_hand(&mut self) {
        ...
        // 下盲注
        let sb_index = self.next_active_player(self.dealer_index).unwrap_or(self.dealer_index);
    ...
    fn evaluate_hand(&self, cards: &[Card]) -> HandEvaluation {
        ...
        for i in 0..cards.len() {
            for j in (i + 1)..cards.len() {
                let mut five_cards: Vec<Card> = cards.iter().enumerate()
    ...
    fn evaluate_five_cards(&self, cards: &[Card]) -> HandEvaluation {
        ...
        let kickers: Vec<u8> = rank_counts.iter().flat_map(|(count, rank)| {
            std::iter::repeat(*rank).take(*count as usize)
        }).collect();
...
impl GameEngine for TexasHoldemEngine {
    ...
    fn step(&mut self, actor_id: &str, action: serde_json::Value) -> Result<Vec<EngineEvent>, String> {
        ...
        // 如果游戏已结束
        if self.finished || self.phase == GamePhase::Finished {
        ...
        if self.phase == GamePhase::Showdown {
            ...
            let winner_share = self.pot / winners.len() as u32;
        ...
        // 验证是否轮到该玩家
        let player_index = self.players.iter().position(|p| p.id == actor_id)
        ...
        if player_index != self.active_index {
            return Err(format!(
                "还没轮到 {} 行动，当前应该是 {}",
                actor_id,
                self.players[self.active_index].id
            ));
        }
        ...
        let player_action = match action_str {
            ...
            _ => return Err(format!("未知动作: {}", action_str)),
        };
        ...
        if self.non_folded_count() <= 1 {
            let winner_id = self.players.iter()
                .find(|p| !p.folded)
                .map(|p| p.id.clone())
                .unwrap_or_default();
        ...
        if let Some(next) = self.next_active_player(self.active_index) {
            ...
            // 检查本轮下注是否完成
            if self.all_bets_equal() && self.active_players_count() <= 1 {
                // 只有 all-in 的玩家还在，直接推进到摊牌
            ...
            } else if self.all_bets_equal() {
                // 检查是否所有人（除了 all-in）都已经行动过
                // 简化处理：如果当前玩家的下注等于最大下注，推进阶段
                self.advance_phase();

                if self.phase == GamePhase::Showdown {
                    let winners = self.showdown();
                    let winner_share = if winners.is_empty() { 0 } else { self.pot / winners.len() as u32 };
                    for winner_id in &winners {
                        if let Some(player) = self.players.iter_mut().find(|p| p.id == *winner_id) {
                            player.chips += winner_share;
                        }
    ...
    fn to_json(&self) -> serde_json::Value {
        ...
        let players_json: Vec<serde_json::Value> = self.players.iter().map(|p| {
            serde_json::json!({
                "id": p.id,
                "kind": p.kind,
                "chips": p.chips,
    ...
    fn current_actor(&self) -> Option<String> {
        ...
        self.players.get(self.active_index).map(|p| p.id.clone())
    }
...
mod tests {
    ...
    fn test_hand_evaluation() {
        ...
        let eval = engine.evaluate_five_cards(&royal_flush);
        assert_eq!(eval.category, HandRankCategory::RoyalFlush);
        ...
        let eval = engine.evaluate_five_cards(&four_kind);
        assert_eq!(eval.category, HandRankCategory::FourOfAKind);
    }
}
